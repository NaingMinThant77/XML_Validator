...
// Function to peek at the top tag of the stack
...
int skipComment(FILE* file) {
    int c;
    while ((c = fgetc(file)) != EOF) { // End of File // returns the ASCII code of the character
...
int validateXML(FILE* file) {
    ...
    while ((c = fgetc(file)) != EOF) {
        if (c == '<') {
            ...
            if (tag[tagLength - 1] == '/') continue;

            if (isClosingTag) {
                if (isEmpty(&stack) || !tagsMatch(peek(&stack), tag)) {
...
int main() {
    FILE* file = fopen("Assignment/2Assignment/invalid.xml", "r"); // "r" stands for read only mode
    if (file == NULL) {
